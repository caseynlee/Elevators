...
void Game::playGame(bool isAIModeIn, ifstream& gameFile) {
    ...
    while (gameFile >> command) {
        Person p(command);
        for (int i = building.getTime(); i < p.getTurn(); i++) {
...
bool Game::isValidPickupList(const string& pickupList, const int pickupFloorNum) const {
    ...
    for (int i = 0; i < pickupList.length(); i++) {
        ...
        for (int j = i + 1; j < pickupList.length(); j++) {
            if (pickupList[i] - '0' == pickupList[j] - '0') {
                return false;
            }
        }
    }
    Floor level = building.getFloorByFloorNum(pickupFloorNum);
    int num = level.getNumPeople();
    int max = 0;
    for (int k = 0; k < pickupList.length(); k++) {
        if (pickupList[k] - '0' > max) {
            max = pickupList[k] - '0';
        }
    }
    if (max >= num) {
    ...
    if (level.getPersonByIndex(pickupList.at(0) - '0').getTargetFloor() - level.getPersonByIndex(pickupList.at(0) - '0').getCurrentFloor() > 0) {
        status = true;
    }
    else {
        status = false;
    ...
    for (int i = 1; i < pickupList.length(); i++) {
        bool status2;
        if (level.getPersonByIndex(pickupList.at(i) - '0').getTargetFloor() - level.getPersonByIndex(pickupList.at(i) - '0').getCurrentFloor() > 0) {
            status2 = true;
        }
        else {
            status2 = false;
